...
def infer_on_video(args, model):
    ...
    while cap.isOpened():
        ...
        if plugin.wait() == 0:
            ...
            payload_classes = json.dumps({"class_names": class_names})
            payload_speedometer = json.dumps({"speed": speed})
            client.publish("class", payload_classes)
            client.publish("speedometer", payload_speedometer)
